...
def train_model(plane, epoch,batch_size,early_stop,continue_train):
    ...
    for i in range(epoch):
        ...
        if(best_acc>acc):
            ...
            early_stop_count=0
            # save model
            pt.save(model, "save_MLP_new_model/mlp_model_plane" + str(plane) + ".pt")
        else:
            early_stop_count=early_stop_count+1
        print("*** Epoch "+str(i)+" val MES: " + str(acc))

    pickle.dump(loss_list, open('save_MLP_new_model/plane'+str(plane)+'loss_list.seqs', 'wb'), -1)
...
def train_model_uncertainty(plane, epoch,batch_size,early_stop,continue_train,uncertainty_model_number):
    ...
    pickle.dump(loss_list, open('save_MLP_new_model/plane'+str(plane)+'loss_list.seqs', 'wb'), -1)
    pickle.dump(uncertainty_list, open('save_MLP_new_model/plane' + str(plane) + 'uncertainty_list.seqs', 'wb'), -1)
    print("[***] Best MES is " + str(best_acc))
...
def test(X,Y,plane):
    ...
    # load model
    load_model=pt.load("save_MLP_new_model/mlp_model_plane" + str(plane) + ".pt")
    lossfunc = pt.nn.MSELoss()
