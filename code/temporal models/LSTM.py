...
class LSTM(object):
    ...
    def __init__(self, input_dim, output_dim, hidden_dim, fc_dim, train):
        ...
        self.input = tf.placeholder('float',
                                    shape=[None, None, self.input_dim])  # [batch size x seq length x input dim]
    ...
    def get_cost_acc(self):
        pre = self.get_outputs()
        loss= tf.reduce_mean(tf.losses.mean_squared_error(self.labels, pre))

        return loss, pre, self.labels
